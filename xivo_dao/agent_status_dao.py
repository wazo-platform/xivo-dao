...
def get_status_by_state_interface(session, state_interface, tenant_uuids=None):
    login_status = _get_login_status_by_state_interface(
        session, state_interface, tenant_uuids=tenant_uuids
    )
    if not login_status:
        return None
...
def get_statuses(session, tenant_uuids=None):
    query = session.query(
        ...
        case([(AgentLoginStatus.agent_id == None, False)], else_=True).label(
            'logged'
        ),  # noqa
