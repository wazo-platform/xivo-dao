...
class TestStatAgentDAO(DAOTestCase):
    ...
    def test_mark_recreated_agents_with_same_number_as_deleted(self):
        ...
        result = self._fetch_stat_agents()
        assert_that(result, contains_inanyorder(('Agent/1', 'tenant', 999, True)))
    ...
    def test_mark_non_confd_agents_as_deleted(self):
        ...
        with flush_session(self.session):
            stat_agent_dao._mark_non_confd_agents_as_deleted(
                self.session, confd_agents_by_key
            )
    ...
    def _fetch_stat_agents(self):
        return [
            ...
            for name, tenant_uuid, agent_id, deleted in self.session.query(
                StatAgent.name,
                StatAgent.tenant_uuid,
                StatAgent.agent_id,
                StatAgent.deleted,
            ).all()
        ]
